...
from models.users import user_groups

# Verbindungstabelle für die Beziehung zwischen Gruppen und API-Berechtigungen
group_api_permissions = Table(
    'group_api_permissions',
    Base.metadata,
    Column('group_id', Integer, ForeignKey('groups.id'), primary_key=True),
    Column('api_permission_id', Integer, ForeignKey('api_permissions.id'), primary_key=True)
)

# Verbindungstabelle für die Beziehung zwischen Gruppen und Dashboard-Modulen
group_dashboard_modules = Table(
    'group_dashboard_modules',
    Base.metadata,
    Column('group_id', Integer, ForeignKey('groups.id'), primary_key=True),
    Column('dashboard_module_id', Integer, ForeignKey('dashboard_modules.id'), primary_key=True)
)
...
class Group(Base):
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "api_permissions": [perm.endpoint for perm in self.api_permissions],
            "dashboard_modules": [module.module_id for module in self.dashboard_modules]
        }
