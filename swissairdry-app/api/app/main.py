...
Hauptmodul mit FastAPI-Anwendung.
...
# Nextcloud-Verbindungs-Endpunkt (optional)
...
async def nextcloud_connect():
    ...
    if config.STANDALONE_MODE:
        return {
            "status": "ok",
            "message": "Läuft im Standalone-Modus ohne Nextcloud-Integration",
            "standalone_mode": True
        }
    
    # Im integrierten Modus prüfen wir die Nextcloud-Verbindung
...
from routers import admin_ui, data_processing

app.include_router(admin_ui.router, prefix=config.ADMIN_PREFIX)
app.include_router(data_processing.router, prefix=f"{config.API_PREFIX}/data")

# Weiteren Router für die Standalone-Frontend-Integration hinzufügen
if config.STANDALONE_MODE:
    from routers import frontend_integration
    app.include_router(frontend_integration.router, prefix=f"{config.API_PREFIX}/frontend")
